...
export default (function () {
  ...
  function createMoveHash(initState, validMoves, hashFunc, moveFunc) {
    ...
    for (var i = 0; i < states.length; i++) {
      ...
      if (i % 10000 == 9999) {
        SCRAMBLE_DEBUG &&
          console.log(i, "states scanned, tt=", +new Date() - tt);
      }
  ...
  CubieCube.prototype.toPerm = function (
    cFacelet,
    eFacelet,
    ctFacelet,
    withOri,
  ...
  function createPrun(prun, init, size, maxd, doMove, N_MOVES, N_POWER, N_INV) {
    ...
    for (var l = 0; l <= maxd; l++) {
      ...
      SCRAMBLE_DEBUG && console.log("[prun]", done);
    }
  }
  ...
  _.toStr = function (sol, move_map, power_map) {
    return sol
      .map((move) => move_map[move[0]] + power_map[move[1]])
      .join(" ")
      .replace(/ +/g, " ");
  };
  ...
  _.updatePrun = function (targetDepth) {
    ...
    for (var depth = this.prunDepth + 1; depth <= targetDepth; depth++) {
      if (this.prevSize >= this.MAX_PRUN_SIZE) {
        SCRAMBLE_DEBUG &&
          console.log("[gSolver] skipPrun", depth, this.prunTableSize);
        break;
      }
      var t = +new Date();
      if (depth < 1) {
  ...
  var randGen = (function () {
    ...
    if (typeof crypto != "undefined" && crypto.getRandomValues) {
      ...
      SCRAMBLE_DEBUG && console.log("[mathlib] use crypto seed", seed);
    } else {
      SCRAMBLE_DEBUG && console.log("[mathlib] use datetime seed", seed);
    }
    setSeed(256, seed);
