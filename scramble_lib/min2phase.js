export default min2phase = (function () {
  ...
  CubieCube.EdgeConjugate = function (a, idx, b) {
    ...
    for (var ed = 0; ed < 12; ed++) {
      b.ea[ed] =
        sinv.ea[a.ea[s.ea[ed] & 0xf] & 0xf] ^
        (a.ea[s.ea[ed] & 0xf] & 0x10) ^
        (s.ea[ed] & 0x10);
    }
  };
  ...
  CoordCube.prototype.calcPruning = function (isPhase1) {
    this.prun = Math.max(
      getPruningMax(
        ...
        SliceTwstPrun,
        (this.twst >> 3) * N_SLICE +
          SliceConj[(this.slice << 3) | (this.twst & 7)],
      ),
      getPruningMax(
        SliceFlipPrunMax,
        SliceFlipPrun,
        (this.flip >> 3) * N_SLICE +
          SliceConj[(this.slice << 3) | (this.flip & 7)],
      ),
      USE_CONJ_PRUN
        ? getPruningMax(
            TwstFlipPrunMax,
      ...
      USE_TWST_FLIP_PRUN
        ? getPruningMax(
            TwstFlipPrunMax,
            TwstFlipPrun,
            ((this.twst >> 3) << 11) | FlipS2RF[this.flip ^ (this.twst & 7)],
          )
        : 0,
    );
  };
  ...
  CoordCube.prototype.doMovePrun = function (cc, m, isPhase1) {
    ...
    this.flip =
      FlipMove[(cc.flip >> 3) * N_MOVES + Sym8Move[(m << 3) | (cc.flip & 7)]] ^
      (cc.flip & 7);
    this.twst =
      TwstMove[(cc.twst >> 3) * N_MOVES + Sym8Move[(m << 3) | (cc.twst & 7)]] ^
      (cc.twst & 7);
    this.prun = Math.max(
      getPruningMax(
        SliceTwstPrunMax,
  ...
  CoordCube.prototype.doMovePrunConj = function (cc, m) {
    ...
    this.flipc =
      FlipMove[
        (cc.flipc >> 3) * N_MOVES + Sym8Move[(m << 3) | (cc.flipc & 7)]
      ] ^
      (cc.flipc & 7);
    this.twstc =
      TwstMove[
        (cc.twstc >> 3) * N_MOVES + Sym8Move[(m << 3) | (cc.twstc & 7)]
      ] ^
      (cc.twstc & 7);
  ...
  Search.prototype.solution = function (
    ...
    for (var i = 0; i < 3; i++) {
      if (firstAxisFilter !== undefined) {
        this.firstFilters[i] |=
          0xe07 << (~~(urfMove[(3 - i) % 3][firstAxisFilter * 3] / 3) * 3);
        this.lastFilters[i + 3] |=
          0xe07 << (~~(urfMove[(3 - i) % 3][firstAxisFilter * 3] / 3) * 3);
      }
      if (lastAxisFilter !== undefined) {
        this.lastFilters[i] |=
          0xe07 << (~~(urfMove[(3 - i) % 3][lastAxisFilter * 3] / 3) * 3);
        this.firstFilters[i + 3] |=
          0xe07 << (~~(urfMove[(3 - i) % 3][lastAxisFilter * 3] / 3) * 3);
      }
    }
    this.initSearch();
  ...
  Search.prototype.verify = function (facelets) {
    ...
    if (
      (getNParity(getNPermFull(this.cc.ea, 12), 12) ^
        getNParity(this.cc.getCPerm(), 8)) !=
      0
    ) {
  ...
  Search.prototype.phase1PreMoves = function (maxl, lm, cc) {
    ...
    if (
      ...
      this.allowShorter =
        this.depth1 == MIN_P1LENGTH_PRE && this.preMoveLen != 0;
  ...
  Search.prototype.initPhase2Pre = function () {
    ...
    for (var i = this.valid1; i < this.depth1; i++) {
      CubieCube.CornMult(
        this.phase1Cubie[i],
        moveCube[this.move[i]],
        this.phase1Cubie[i + 1],
      );
      CubieCube.EdgeMult(
        this.phase1Cubie[i],
        moveCube[this.move[i]],
  ...
  Search.prototype.initPhase2 = function (phase2Cubie) {
    ...
    var prun = Math.max(
      getPruningMax(
        ...
        p2edge * N_COMB +
          CCombPConj[
            ((Perm2CombP[p2corn] & 0xff) << 4) | SymMultInv[p2esym][p2csym]
          ],
      ),
      getPruningMax(
        MCPermPrunMax,
  ...
  Search.prototype.phase2 = function (
    edge,
    esym,
    corn,
    csym,
    ...
    if (
      this.depth1 == 0 &&
      depth == 1 &&
      ((this.firstFilter >> ud2std[lm]) & 1) != 0
    ) {
      return -1;
    }
    if (
      ...
      corn == 0 &&
      mid == 0 &&
      (this.preMoveLen > 0 || ((this.lastFilter >> ud2std[lm]) & 1) == 0)
    ) {
      return maxl;
    }
    ...
    for (var m = 0; m < 10; m++) {
      ...
      if (
        getPruningMax(
          MCPermPrunMax,
          MCPermPrun,
          cornx * N_MPERM + MPermConj[(midx << 4) | csymx],
        ) >= maxl
      ) {
        continue;
      }
      ...
      if (
        getPruningMax(
          ...
          edgex * N_COMB +
            CCombPConj[
              ((Perm2CombP[cornx] & 0xff) << 4) | SymMultInv[esymx][csymx]
            ],
        ) >= maxl
      ) {
        continue;
      ...
      if (
        getPruningMax(
          ...
          (edgei >> 4) * N_COMB +
            CCombPConj[
              ((Perm2CombP[corni >> 4] & 0xff) << 4) |
                SymMultInv[edgei & 0xf][corni & 0xf]
            ],
        ) >= maxl
      ) {
        continue;
  ...
  Search.prototype.solutionToString = function () {
    var sb = "";
    var urf =
      (this.verbose & INVERSE_SOLUTION) != 0
        ? (this.urfIdx + 3) % 6
        : this.urfIdx;
    if (urf < 3) {
  ...
  function initBasic() {
    ...
    function initSym2Raw(
      ...
      var conjFunc =
        coord != 1 ? CubieCube.EdgeConjugate : CubieCube.CornConjugate;
    ...
    for (var i = 0; i < N_COMB; i++) {
      ...
      for (var j = 0; j < N_MOVES2; j++) {
        ...
        CCombPMove[i * N_MOVES2 + j] =
          d.getCComb() + 70 * (((0xa5 >> j) & 1) ^ ~~(i / 70));
      }
      for (var j = 0; j < 16; j++) {
      ...
      for (var i = 0; i < N_SIZE; i++, val >>= 4) {
        ...
        for (var m = 0; m < N_MOVES; m++) {
          ...
          if (ISTFP) {
            ...
            rawx =
              RawConj[
                (RawMove[raw * N_MOVES + m] << SYM_SHIFT) | (symx & SYM_MASK)
              ];
          }
          symx >>= SYM_SHIFT;
          ...
          for (var j = 1, selfSym = SelfSym[symx]; (selfSym >>= 1) != 0; j++) {
            ...
            if (ISTFP) {
              ...
              idxx +=
                RawConj[
                  (rawx << SYM_SHIFT) | (j ^ ((SYM_E2C_MAGIC >> (j << 1)) & 3))
                ];
            }
